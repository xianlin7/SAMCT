...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        #x self.samct_adapter1 = Adapter(D_features=embed_dim)
        self.samct_trans2cnn1 = Trans2CNN(dimcnn=2*base_dim, dimtrans=embed_dim, cnn_patch_size=patch_size//2)
        #self.samct_exchange1 = ExchangeBlock(dimq=embed_dim, dimkv=2*base_dim, num_heads=8, qkv_bias=qkv_bias, cnn_patch_size=patch_size//2)
        ...
        #x self.samct_adapter2 = Adapter(D_features=embed_dim)
        self.samct_trans2cnn2 = Trans2CNN(dimcnn=4*base_dim, dimtrans=embed_dim, cnn_patch_size=patch_size//4)
        #self.samct_exchange2 = ExchangeBlock(dimq=embed_dim, dimkv=4*base_dim, num_heads=8, qkv_bias=qkv_bias, cnn_patch_size=patch_size//4)
        ...
        #x self.samct_adapter3 = Adapter(D_features=embed_dim)
        self.samct_trans2cnn3 = Trans2CNN(dimcnn=8*base_dim, dimtrans=embed_dim, cnn_patch_size=patch_size//8)
        #self.samct_exchange3 = ExchangeBlock(dimq=embed_dim, dimkv=8*base_dim, num_heads=8, qkv_bias=qkv_bias, cnn_patch_size=patch_size//8)
        ...
        #x self.samct_adapter4 = Adapter(D_features=embed_dim)
        self.samct_trans2cnn4 = Trans2CNN(dimcnn=16*base_dim, dimtrans=embed_dim, cnn_patch_size=patch_size//16)
        #self.samct_exchange4 = ExchangeBlock(dimq=embed_dim, dimkv=16*base_dim, num_heads=8, qkv_bias=qkv_bias, cnn_patch_size=patch_size//16)
        ...
        # self.samct_Adapter1 = Adapter(embed_dim, skip_connect=True)
        # self.samct_Adapter2 = Adapter(embed_dim, skip_connect=True)
        # self.samct_Adapter3 = Adapter(embed_dim, skip_connect=True)
        # self.samct_Adapter4 = Adapter(embed_dim, skip_connect=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x16 = cnnx4 # (b 768 16 16)
        x32 = self.samct_up1(x16, cnnx3) # (b 384 32 32)
        x64 = self.samct_up2(x32, cnnx2) # (b 192 64 64)
        x128 = self.samct_up3(x64, cnnx1) # (b 96 128 128)
        x256 = self.samct_up4(x128, cnnx) # (b 48 256 256)
        x = self.samct_neck(x256) # (b 32 256 256)

        transx = self.neck(transx) # (b 256 16 16)
        
        return transx, x, x256, x128, x64, x32, x16
...
class ExchangeBlock(nn.Module):
    ...
    def __init__(
        ...
        #------ for trans2cnn ------
        self.trans2cnn_sa = SpatialAttention(kernel_size=3)
        self.trans2cnn_fc = nn.Conv2d(dimkv, dimq, kernel_size=1, bias=False)
        self.trans2cnn_scale = dimq**0.5
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x_trans: torch.Tensor, x_cnn: torch.Tensor) -> torch.Tensor:
        ...
        attn = torch.matmul(q, k.transpose(-1, -2)) * self.scale
        #attn = attn.softmax(dim=-1)
        attn = softmax_one(attn, dim=-1) # b g N N
        out = torch.matmul(attn, v)
        out = rearrange(out, 'B g N d -> B N (g d)')
        out = self.proj(out)
        out = rearrange(out, '(b H W) N c -> b H W (N c)', H=H, W=W)
        out = self.combine(x_trans + out)

        #------------- trans2cnn -----------------
        trans = rearrange(x_trans, 'b H W c->b c H W')
        sa = self.trans2cnn_sa(trans) # b 1 H W
        sa = rearrange(sa, 'b g H W -> (b H W) g') # (BHW 1)
        t2c_attn = torch.mean(attn, dim=1) # BHW 1 NN
        t2c_attn = 0.5 + self.sigmoid(t2c_attn - 1/(self.window_size*self.window_size*1.0))
        t2c_attn = t2c_attn * sa[:, :, None]
        t2c_attn = rearrange(t2c_attn, '(b H W) g (m n) -> b g (H m) (W n)', m=self.window_size, H=H, W=W)
        out2 = x_cnn * t2c_attn
        return out + x_trans, out2 + x_cnn

...
def smooth_l1_loss(x, t, sigma=1):
    sigma2 = sigma ** 2
    diff = (x - t)
    abs_diff = diff.abs()
...
class auto_prompt_loss(nn.Module):
    def __init__(self, pos_pt_num=5, neg_pt_num=0.5):
        """
        DO NOT APPLY NONLINEARITY IN YOUR NETWORK!
        THIS LOSS IS INTENDED TO BE USED FOR BRATS REGIONS ONLY
        :param soft_dice_kwargs:
        ...
        :param aggregate:
        """
        super(auto_prompt_loss, self).__init__()
        self.pos_pt_num = pos_pt_num
        self.neg_pt_num = neg_pt_num

    def forward(self, net_output):
        pos_pt_embed, neg_pt_embed, box_embed = net_output["pos_pt_embed"], net_output["neg_pt_embed"], net_output["box_embed"]
        pos_pt_se, neg_pt_se, box_se =  net_output["pos_pt_se"], net_output["neg_pt_se"], net_output["box_se"] 
        if box_se.shape[1] > 0:
            loss_box = smooth_l1_loss(box_embed, box_se)
        else:
            loss_box = smooth_l1_loss(neg_pt_embed, neg_pt_se)
        loss_neg = smooth_l1_loss(neg_pt_embed, neg_pt_se)
        loss_pos = smooth_l1_loss(pos_pt_embed, pos_pt_se)
        # pos = pos_pt_embed * pos_pt_se
        # pos = torch.sum(pos, dim=-1) # [b k]
        # pos = torch.exp(pos)
        # neg = neg_pt_embed * neg_pt_se
        # neg = torch.sum(neg, dim=-1)
        # neg = torch.exp(neg)
        # loss_pos = -torch.log(torch.sum(pos, dim=-1) / (torch.sum(pos, dim=-1) + torch.sum(neg, dim=-1)))
        # loss_pos = loss_pos.mean()
        # loss_neg = -torch.log(torch.sum(neg, dim=-1) / (torch.sum(pos, dim=-1) + torch.sum(neg, dim=-1)))
        # loss_neg = loss_neg.mean()
        #print(loss_pos, loss_neg, loss_box)
        return 0.8*loss_box + 0.1*loss_neg + 0.1*loss_pos

...
from utils.data import JointTransform2D, ImageToImage2D
...
def main():
    ...
    parser.add_argument('--modelname', default='SAMCT', type=str, help='type of model, e.g., SAM, SAMCT, SAMUS, MSA, SAMed, SAMMed2D...')
    ...
    parser.add_argument('--task', default='INSTANCE', help='task or dataset name') # Modify according to the task
    ...
    tf_train = JointTransform2D(img_size=args.encoder_input_size, low_img_size=args.low_image_size, ori_size=opt.img_size, crop=opt.crop, p_flip=0.0, p_rota=0.0, p_scale=0.5, p_gaussn=0.0,
    ...
    train_dataset = ImageToImage2D(opt.data_path, opt.train_split, tf_train, img_size=args.encoder_input_size) # return image, mask, and filename
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_idx, (datapack) in enumerate(trainloader):
            ...
            train_loss = criterion(pred, masks) # masks b 1 h w, pred b c h w
            # ---------------------------------- backward ---------------------------------
            optimizer.zero_grad()
            train_loss.backward()
            optimizer.step()
            ...
            # -------------------------------------------------- evaluate in middle iteraction --------------------------------------------------
            # if (epoch_iter_number+1) % 500 == 0:
            #     model.eval()
            #     dices, mean_dice, _, val_losses = get_eval(valloader, model, criterion=criterion, opt=opt, args=args)
            #     keep_number = keep_number+1
            #     if mean_dice > best_dice_iter:
            #         best_dice_iter = mean_dice
            #         timestr = time.strftime('%m%d%H%M')
            #         if not os.path.isdir(opt.save_path):
            #             os.makedirs(opt.save_path)
            #         save_path = opt.save_path + args.modelname + opt.save_path_code + '%s' % timestr + '_epoch_' + str(epoch) + '_iter_' + str(keep_number) + '_' + str(best_dice_iter)
            #         torch.save(model.state_dict(), save_path + ".pth", _use_new_zipfile_serialization=False)
            # epoch_iter_number = epoch_iter_number + 1

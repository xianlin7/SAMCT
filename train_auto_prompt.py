...
from utils.loss_functions.sam_loss import get_criterion, auto_prompt_loss
...
def main():
    ...
    opt.eval_mode = "mask_patient"
    opt.load_path = "checkpoints/SAMCT-CT5Mv1.pth"
    ...
    prompt_loss_func = auto_prompt_loss()
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_idx, (datapack) in enumerate(trainloader):
            ...
            gt_class_label = datapack['p_label'].to(dtype = torch.long, device=opt.device)
            # ---------------------------------- forward ----------------------------------
            ...
            mask_loss = criterion(pred, masks) # masks b 1 h w, pred b c h w
            prompt_loss = prompt_loss_func(pred)
            class_score = pred['class_score']
            gt_class_label[gt_class_label < 1] = 0
            class_loss = F.cross_entropy(class_score.view(-1, 2), gt_class_label[:, 0].view(-1))
            #train_loss = mask_loss#  + 0.2*prompt_loss
            #train_loss = 0.8*mask_loss + 0.2*class_loss
            train_loss = 0.8*mask_loss + 0.1*class_loss +0.1*prompt_loss
            print(train_loss)
            # ---------------------------------- backward ---------------------------------
            optimizer.zero_grad()
            train_loss.backward()
